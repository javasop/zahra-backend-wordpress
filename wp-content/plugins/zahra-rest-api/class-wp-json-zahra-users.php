...
//this will handle adding new zahra members ...
class Zahra_Users {
...
//            }
        ...
        //add meta to the user ...
        $d = $this->addMeta($data, $user_id);
        ...
        //todo: seperate email sending in a seperate hook?
        //http://galamr.com/zahrahControl/wp-content/uploads/2014/08/image00.png
...
//        }
        ...
        $message = "مرحبا,
يسرنا أنه تم بنجاح استلام طلب اشتراكك في العضوية 
ملاحظة :
يمكنك سداد اشتراك العضوية نقداً أو شيك باسم جمعية زهرة لسرطان الثدي أو الإيداع في حسابات الجمعية خلال أسبوع واحد بعد ارسال طلب الإشتراك 

كما يمكن إرسال صورة من السداد على الفاكس: 0112812841   أو البريد الإلكتروني: members@zahra.org.sa
ان كان لديك أي استفسارات يرجى مراسلتنا عبر البريد الإلكتروني  members@zahra.org.sa 
شكراً لك !";
        
        
        $headers = 'From: Zahra Members <members@zahra.org.sa>' . "\r\n";
        
        $attachement =  WP_CONTENT_DIR . '/uploads/2014/08/image00.png';
        
        wp_mail( $data['email'], "متجر زهرة ", $message, $headers,$attachement);
     ...
     *
     * @param int $id
     * @param bool force        "father":"taleb",
        "grandfather":"abdulkarim",
        "issue_city":"riyadh",
        "issue_date":"1/1/2014",
        "birthdate":"1/1/1989",
        "birthplace":"taif",
    ...
    public function addMeta($data, $user_id) {

        update_user_meta($user_id, "dbem_phone", $data['phone']);
        ...
        update_user_meta($user_id, "district", $data['district']);
        update_user_meta($user_id, "national_id", $data['national_id']);
        update_user_meta($user_id, "father", $data['father']);
        update_user_meta($user_id, "grandfather", $data['grandfather']);
        update_user_meta($user_id, "issue_city", $data['issue_city']);
        update_user_meta($user_id, "issue_date", $data['issue_date']);
        update_user_meta($user_id, "birthdate", $data['birthdate']);
        update_user_meta($user_id, "birthplace", $data['birthplace']);
